...
class AsynHandle { 
    ...
    //请求函数,只请求,不返回 
    public function Request($url, $post=array(), $cookie=array(), $timeout=3){
        $this->url     = $url;
        $this->cookie  = $cookie;
        $this->post    = $post;
        ...
        //返回状态检测,301、302重定向处理
        // if (fgets($this->fop)){
        //     $checkHttp = $this->checkRecvHeaderRequest(fgets($this->fop));
        // }      
    ...
    //获取函数,请求并返回 
    public function Get($url, $post=array(), $cookie=array(), $timeout=3){
        $this->url     = $url;
        $this->cookie  = $cookie;
        $this->post    = $post;
        $this->timeout = $timeout;
        ...
        if(!$status['timed_out']){ 
            ...
            //返回状态检测,301、302重定向处理
            $checkHttp = $this->checkRecvHeaderGet($h);
    ...
    private function analyzeUrl(){ 
        ...
        $this->requestUri = $url_array['path'] ? $url_array['path'].($url_array['query'] ? '?'.$url_array['query'] : '') : '/'; 
    ...
    //返回状态检测,301、302重定向处理 Get
    private function checkRecvHeaderGet($header){ 
        if(strstr($header,' 301 ') || strstr($header,' 302 ')){//重定向处理 
            ...
            //重新请求
            $obj          = new AsynHandle(); 
            $result       = $obj->Get($url, $this->post, $cookie); 
            $this->result = $result; 
            return $result;
    ...
    //返回状态检测,301、302重定向处理 
    private function checkRecvHeaderRequest($header){ 
        if(strstr($header,' 301 ') || strstr($header,' 302 ')){//重定向处理 
            preg_match("/Location:(.*?)$/im",$header,$match); 
            $url = trim($match[1]); 
            preg_match("/Set-Cookie:(.*?)$/im",$header,$match); 
            ...
            //重新请求
            $obj          = new AsynHandle(); 
            $result       = $obj->Request($url, $this->post, $cookie); 
            $this->result = $result; 
            return $result;

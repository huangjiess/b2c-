...
$batchcode_str = i2post("order_batchcode","");
$batchcode_arr = explode('_',$batchcode_str);
$extra['batchcode'] = $batchcode_arr[0];
//订货系统参数
$extra['user_id'] = i2post("pay_user_id","-1");
if($industry_type=="orderingretail"){
    $extra['opt'] = i2post("opt","");
    $extra['is_virtual'] = i2post("is_virtual","");
}
$op = i2post("op","pay_way");
if($op == "pay_way"){ //显示支付方式
    $result = $industry_obj->getPayType($industry_type ,$client,$extra);
    ...
    if(!empty($exits_user)){
        if($pay_type == "card"){
            ...
            $balance = 0 ;
            $query = "SELECT pa.account as account FROM orderingretail_proxy_account AS pa INNER JOIN orderingretail_proxy AS p ON pa.proxy_id = p.id WHERE p.user_id = ".$user_id." AND p. STATUS = 'checked' ";
            $query.=" AND p.isvalid = TRUE and p.customer_id=".$customer_id." LIMIT 1;";
    ...
    $query_item = "select or_product_id,proids,product_id,counts from orderingretail_order_item where batchcode='".$order_batchcode."' and customer_id=".$customer_id;
    ...
    while($row = mysql_fetch_object($result_item)){
        ...
        $counts = $row->counts;
        //查询每件产品虚拟库存数量
        $product_query = "select id,virtual_store_count from orderingretail_proxy_product where or_product_id='".$or_product_id."' and proids='".$proids."' and proxy_id=".$proxy_id." limit 1";
